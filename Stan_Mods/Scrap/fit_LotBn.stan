...
parameters {
  ...
  real<lower=0> sig[S]; // standard deviation for lambda per species
...
transformed parameters {
  real mu[N]; //
for(n in 1:N){
  mu[n] = lam[sp[n], r[n], e[n]] - alpha[sp[n], 1, r[n], e[n]]*AA[n] - alpha[sp[n], 2, r[n], e[n]]*CG[n] - alpha[sp[n], 3, r[n], e[n]]*LB[n];
  }
}
...
generated quantities {
  vector[N] log_lik;
  for (n in 1:N) {
    log_lik[n] = normal_lpdf(fec[n] | mu[n], sig[sp[n]]);
  }
}