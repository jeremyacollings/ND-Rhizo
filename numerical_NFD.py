...
import numpy as np # a package for working with arrays and matrices
from scipy.optimize import brentq, fsolve # functions for root finding using Brent's method and Powell's hybrid method respectively
from warnings import warn # function for issuing warnings
...
def NFD_model(f, n_spec = 2, args = (), monotone_f = True, pars = None,
    ...
    # CHECK THAT THERE ARE MORE THAN ONE SPECIES
    if n_spec == 1:
    ...
    # MAKE SURE THAT OBJECTS BROUGHT IN FROM R ARE IN PROPER NUMPY TYPES
    if from_R:
    ...
    # __input_check__ IS DEFINED BELOW
    
    # IF EXPERIMENTAL, FILL IN c VALUES AS 1 AND FILL IN INVADER GROWTH RATE VALUES
    # AS EQUILIBRIUM GROWTH RATES FOR EACH SPECIES
    if experimental:
    ...
    # IF NOT EXPERIMENTAL, GET EQUILIBRIA DENSITIES AND INVADER GROWTH RATES
    # USING THE preconditioner FUNCTION
    if not experimental:
        ...
        # preconditioner IS DEFINED BELOW
        
    # list of all species
    l_spec = list(range(n_spec))
    
    # GET CONVERSION FACTOR TRIANGULAR MATRIX
